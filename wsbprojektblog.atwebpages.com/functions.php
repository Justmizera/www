<?php 
	...
	$user_id = 1;
	// connect to database
	$db = mysqli_connect("fdb34.awardspace.net", "4016277_justyna", "Projekt123!", "4016277_justyna");
	// get post with id 1 from database
	$post_query_result = mysqli_query($db, "SELECT * FROM posts WHERE id=1");
	$post = mysqli_fetch_assoc($post_query_result);

	// Get all comments from database
	$comments_query_result = mysqli_query($db, "SELECT * FROM comments_comment WHERE post_id=" . $post['id'] . " ORDER BY created_at DESC");
	...
	{
		global $db;
		$result = mysqli_query($db, "SELECT username FROM users_comment WHERE id=" . $id . " LIMIT 1");
		// return the username
	...
	{
		global $db;
		$result = mysqli_query($db, "SELECT * FROM replies_comment WHERE comment_id=$id");
	...
	{
		global $db;
		$result = mysqli_query($db, "SELECT COUNT(*) AS total FROM comments_comment");
